...
class StudentAnswerPeerReviewFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val mStudentPeerReviewQuestionsRecyclerView: RecyclerView = view.findViewById(R.id.student_peer_review_questions_recycler_view)
        ...
        val mStudentPeerReviewQuestionsAdditionalComments: EditText = view.findViewById(R.id.student_peer_review_additional_comments)
        val mStudentPeerReviewQuestionsTargetReviewedLabel: TextView = view.findViewById(R.id.student_peer_review_target_reviewed_error_label)
        ...
        db.collection(PeerReview.PEER_REVIEW_COLLECTION).document(arguments!!.getString(SUBJECT_NAME).toString()).collection(arguments!!.getString(
            ASSIGNMENT_ID).toString()).document(arguments!!.getString(GROUP_ID).toString()).get().addOnSuccessListener {documentSnapshot ->
            ...
            if (mStudentPeerReviewQuestionsTargetSpinner.selectedItem.toString() == "-") {
                mStudentPeerReviewQuestionsTargetReviewedLabel.text = resources.getString(R.string.student_peer_review_invalid_review_target_error)
            }
            ...
            db.collection(PeerReviewQuestionSet.PEER_REVIEW_QUESTION_SET_COLLECTION).document(peerReviewGrouping!!.question_set_name.toString()).get().addOnSuccessListener {
                ...
                mQuestionAdapter = QuestionAdapter(activity, mQuestionSet!!)
                ...
                mStudentPeerReviewQuestionsTargetSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                    ...
                    override fun onItemSelected(
                        ...
                        db.collection(PeerReview.PEER_REVIEW_COLLECTION).document(peerReviewGrouping!!.subject_name.toString()).collection(
                            ...
                            mStudentPeerReviewQuestionsTargetReviewedLabel.text = resources.getString(R.string.student_peer_review_target_reviewed_error)
        ...
        mStudentPeerReviewQuestionSubmitButton.setOnClickListener {
            if (mStudentPeerReviewQuestionsTargetSpinner.selectedItem.toString() == "-") {
                Toast.makeText(activity, R.string.student_peer_review_invalid_review_target_error, Toast.LENGTH_SHORT).show()
            } else if (mStudentPeerReviewQuestionsAdditionalComments.text.isEmpty()) {
                Toast.makeText(activity, R.string.student_peer_review_empty_additional_comments_error, Toast.LENGTH_SHORT).show()
            } else if (review_results.size != peerReviewQuestionSet!!.question_list!!.size || review_results.containsValue("-")) {
                Toast.makeText(activity, R.string.student_peer_review_incomplete_questions_error, Toast.LENGTH_SHORT).show()
            } else {
                var reviewMarks = 0
                ...
                peerReviewResult = PeerReviewResult(student.student_email, mStudentPeerReviewQuestionsTargetSpinner.selectedItem.toString(), review_results,
                    mStudentPeerReviewQuestionsAdditionalComments.text.toString(), true, reviewMarks, peerReviewQuestionSet!!.question_list!!.size * 5)
                db.collection(PeerReview.PEER_REVIEW_COLLECTION).document(peerReviewGrouping!!.subject_name.toString()).collection(
                    peerReviewGrouping!!.assignment_id.toString()).document(peerReviewGrouping!!.group_id.toString()).collection(
    ...
    companion object {
        ...
        fun newInstance(): StudentAnswerPeerReviewFragment = StudentAnswerPeerReviewFragment()
        ...
        fun getPeerReviewGroupingNewInstance(subject_name: String, assignment_id: String, group_id: String): StudentAnswerPeerReviewFragment {
            val fragment = StudentAnswerPeerReviewFragment()
            val args = Bundle()
    ...
    class QuestionAdapter(private val context: FragmentActivity?, private val questions: List<String>): RecyclerView.Adapter<QuestionAdapter.QuestionHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): QuestionHolder {
            return QuestionHolder(LayoutInflater.from(context).inflate(R.layout.list_item_peer_review_question, parent, false))
        }
        ...
        class QuestionHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
            ...
            fun bind(question: String, mapPosition: Int) = with(itemView) {
                val mStudentPeerReviewQuestionsLabel: TextView = itemView.findViewById(R.id.student_peer_review_questions_label)
                ...
                val mChoicesAdapter = ArrayAdapter.createFromResource(context, R.array.student_peer_review_questions_spinner_choices, android.R.layout.simple_spinner_item)
